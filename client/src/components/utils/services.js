...
const AddContact = async (contactObj) =>{
   let res = await axios.post(`${url}/api/contacts`,{...contactObj},Config())
   return res.data;
}
const EditContact = async ({id,finalData}) =>{
  let res = await axios.patch(`${url}/api/contacts/${id}/name_dp`,{...finalData},Config())
  return res.data;
}
const DeleteContact = async (contactId) =>{
  let res = await axios.delete(`${url}/api/contacts/${contactId}`,Config())
  return res.data;
}
const AddUrl = async ({urldata,id}) =>{
  let res = await axios.post(`${url}/api/contacts/${id}/url`,{...urldata},Config())
  return res.data;
}
const EditUrl = async ({urldata,contactid,urlid}) =>{
  let res = await axios.patch(`${url}/api/contacts/${contactid}/url/${urlid}`,{...urldata},Config())
  return res.data;
}
const DeleteUrl = async ({contactid,urlid}) =>{
  let res = await axios.delete(`${url}/api/contacts/${contactid}/url/${urlid}`,Config())
  return res.data;
}
const GetAllContacts = async () =>{
   let res = await axios.get(`${url}/api/contacts`,Config())
   return res.data;
}
